...
HOME_POSITIONS = {
    ...
    SERVO_4_ID: 2805,        # Mid position (Adjusted to be near center of 695-2550 range)
...
FEEDBACK_READ_INTERVAL = 1 # <-- FIX: Ensure position is read every loop for home check
...
def read_servo_position(packet_handler, servo_id):
    ...
    # print(f"Warning: Failed to read position for ID {servo_id}")
    return None
    ...
    while running:
        ...
        if going_home:
            ...
            for state, home_pos in [(base_state, HOME_POSITIONS[BASE_SERVO_ID]),
                ...
                # Check if current position is far from home position (use a slightly larger tolerance)
